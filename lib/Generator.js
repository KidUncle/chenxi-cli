...
class Generator {
  ...
  // 按照顺序对 package.json 中的 key 进行排序
  sortPkg() {
  ...
  // 使用 ejs 解析 lib/generator/xx/template 中的文件
  async resolveFiles() {
    ...
    Object.keys(files).forEach((file) => {
      ...
      if (imports && imports.length > 0) {
        files[file] = runTransformation({ path: file, source: files[file] }, require('./utils/codemods/injectImports'), { imports })
      }

      let injections = this.rootOptions[file]
      injections = injections instanceof Set ? Array.from(injections) : injections
      if (injections && injections.length > 0) {
        files[file] = runTransformation({ path: file, source: files[file] }, require('./utils/codemods/injectOptions'), { injections })
      }
    })
  }
